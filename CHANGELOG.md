...
- Project reads now include the optional server-owned `defaultSystemPresetId`.
  Create and patch schemas reject this field.
...
- `IAQPCalculations_v0` now includes optional `controllingMixtures` (`URT`,
  `EYE`, `CNS`) for ASHRAE 62.1 IAQP additive-mixture exposure. When a mixture
  sets the minimum outdoor airflow, `controllingCompoundId` is omitted.

## [0.14.2] - 2026-09-14

### Added

- `APIExport_v0` and `ExportData_v0` carry optional `studyId` and `studyName`
  for exports generated from a study. Both are omitted for baseline exports.
